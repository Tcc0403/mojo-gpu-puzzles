...
fn axis_sum[
    ...
    stride = TPB // 2
    while stride > 0:
        # Note: This implementation has a potential race condition,
        # where threads simultaneously read from and write to shared memory during the same iteration.
        #               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        #               When?
        # if local_i < stride:
        ...
        # Safer approach: seperate read and write phases
